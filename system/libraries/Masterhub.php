...
require_once('/var/www/html/spencer/app/Mage.php'); //Path to Magento
	...
	//http://amitavroy.com/justread/content/articles/code-igniter-jquery-and-ajax
	//http://mydoubts.in/blog/automatic-dropdown-box-loading-using-codeigniter-and-jqueryajax/
	...
	public function getRiderList($storeid){
		echo $storeid;
		$cond   = '';
		$cond   .=  (!empty($storeid))  ? " AND rider_storeid=".$storeid."" : '';  
		$selQry =  "SELECT rider_name,RegistrationNo FROM `rider` WHERE  status ='1' $cond  ";  
		...
		$riders = array();
		 ...
		 if($collections){
			 ...
			 $riders[$data['RegistrationNo']] = $data['rider_name'];
			 }
		 return $riders;
			}else{
			 return FALSE;
